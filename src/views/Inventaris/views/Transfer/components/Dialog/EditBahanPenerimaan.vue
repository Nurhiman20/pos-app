      ...
      <v-card class="pa-3">
        ...
        <ValidationObserver ref="form" v-slot="{ handleSubmit }">
          <v-form @submit.prevent="handleSubmit(editIngredient)">
            ...
            <ValidationProvider v-slot="{ errors }" name="Dikirim" rules="required">
              <v-text-field
                :error-messages="errors"
                v-model="selectedIngredient.delivered"
                label="Dikirim"
                :suffix="selectedIngredient.ingredient.unit"
                outlined
                dense
...
export default {
  ...
  methods: {
    ...
    totalReceive() {
      let total = 0;
      this.rvDeliver.receive.forEach(rv => {
